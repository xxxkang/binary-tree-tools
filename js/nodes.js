...
function createNodes(arr, cx, cy, radius) {
  ...
  while (i < arr.length) {
    ...
    if (i === 0) {
      ...
      if (i == leftChild(parent(i))) {
        ...
        treeContainer.append("line").call(createLineAttr, "black", nodeData[parent(i)].cx, nodeData[parent(i)].cy, node.cx, node.cy);
  ...
  var nodes = treeContainer.selectAll("circle")
    .data(nodeData)
    .enter()
    .append("circle")
  ...
  var texts = treeContainer.selectAll("text.circle")
    .data(nodeData)
    .enter()
    .append("text")
    .attr("class", "circle")
